        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 mt-1">{user?.displayName}さん（管理者）</p>
            </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
                      <p>
                        ...
                        {new Date(event.scheduledStart).toLocaleString('ja-JP')}
                      </p>
                      <p>
                        <span className="font-medium">終了予定:</span>{' '}
                        {new Date(event.scheduledEnd).toLocaleString('ja-JP')}
                      </p>
                      <p>
                        <span className="font-medium">ラウンド:</span> {event.currentRound} /{' '}
                        {event.maxRounds}
                      </p>
                      <p>
                        <span className="font-medium">参加者:</span>{' '}
                        {event.rooms?.reduce((sum, r) => sum + (r.participants?.length || 0), 0) || 0}人
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => handleDeleteEvent(event.id)}
                    disabled={event.status !== 'waiting' && event.status !== 'finished'}
                    className="ml-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    削除
                  </button>
                </div>
